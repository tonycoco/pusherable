  ...
  def pusherable(pusherable_channel='test_channel')
    ...
    class_eval do
      ...
      def pusherable_trigger_create
        Pusher.trigger(pusherable_channel.to_s, "#{pusherable_class_name}.create", { model_id: self.id })
      end

      def pusherable_trigger_update
        Pusher.trigger(pusherable_channel.to_s, "#{pusherable_class_name}.update", { model_id: self.id })
      end

      def pusherable_trigger_destroy
        Pusher.trigger(pusherable_channel.to_s, "#{pusherable_class_name}.destroy", { model_id: self.id })
      end
    end
  end
end
